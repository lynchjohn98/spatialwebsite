...
import { quizData } from "../../../../library/quiz_data/cutting_planes_cross_quiz";
import { submitStudentQuiz } from "../../../../library/services/student_services/student_quiz";

export default function CuttingPlanesCrossQuiz() {
  ...
  useEffect(() => {
    try {
      ...
      if (sessionStorage.getItem("courseData") !== null) {
        ...
        // Check if quiz settings exist and find the "Cutting Planes and Cross-Sections" quiz
        if (courseData?.settings?.quiz_settings) {
          const cuttingPlanesCrossQuiz = courseData.settings.quiz_settings.find(
            quiz => quiz.name === "Cutting Planes and Cross-Sections"
          );

          if (cuttingPlanesCrossQuiz) {
            // Check visibility
            if (cuttingPlanesCrossQuiz.visibility === "Yes") {
              setQuizVisible(true);
              setIsLoading(false);
            } else {
