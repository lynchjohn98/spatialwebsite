...
import { quizData } from "../../../../library/quiz_data/math_instrument_post";
import { submitStudentPrePostQuiz } from "../../../../library/services/student_services/student_quiz";

export default function MathematicsPostTest() {
  ...
  useEffect(() => {
    try {
      ...
      if (sessionStorage.getItem("courseData") !== null) {
        ...
        // Check if quiz settings exist and find the "Math Instrument Post-Test" quiz
        if (courseData?.settings?.quiz_settings) {
          const mathInstrumentPostTestQuiz = courseData.settings.quiz_settings.find(
            quiz => quiz.name === "Math Instrument Post-Test"
          );

          if (mathInstrumentPostTestQuiz) {
            // Check visibility
            if (mathInstrumentPostTestQuiz.visibility === "Yes") {
              setQuizVisible(true);
              setIsLoading(false);
            } else {
