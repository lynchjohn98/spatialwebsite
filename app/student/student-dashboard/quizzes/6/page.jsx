...
import { quizData } from "../../../../library/quiz_data/isometric_plans_quiz";
import { submitStudentQuiz } from "../../../../library/services/student_services/student_quiz";

export default function IsometricDrawingsQuiz() {
  ...
  useEffect(() => {
    try {
      ...
      if (sessionStorage.getItem("courseData") !== null) {
        ...
        // Check if quiz settings exist and find the "Isometric Drawings" quiz
        if (courseData?.settings?.quiz_settings) {
          const IsometricDrawingsQuiz = courseData.settings.quiz_settings.find(
            quiz => quiz.name === "Isometric Drawings and Coded Plans"
          );

          if (IsometricDrawingsQuiz) {
            // Check visibility
            if (IsometricDrawingsQuiz.visibility === "Yes") {
              setQuizVisible(true);
              setIsLoading(false);
            } else {
