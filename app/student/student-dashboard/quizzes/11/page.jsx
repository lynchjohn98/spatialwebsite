...
export default function RotationMultipleAxisQuiz() {
  ...
  useEffect(() => {
    try {
      ...
      if (sessionStorage.getItem("courseData") !== null) {
        ...
        // Check if quiz settings exist and find the "Rotation Around Multiple Axes" quiz
        if (courseData?.settings?.quiz_settings) {
          const rotationMultipleAxesQuiz = courseData.settings.quiz_settings.find(
            quiz => quiz.name === "Rotation Around Multiple Axes"
          );

          if (rotationMultipleAxesQuiz) {
            // Check visibility
            if (rotationMultipleAxesQuiz.visibility === "Yes") {
              setQuizVisible(true);
              setIsLoading(false);
            } else {
