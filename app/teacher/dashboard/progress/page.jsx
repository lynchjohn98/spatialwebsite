...
const parseAndOrganizeData = (responseData) => {
  ...
  const processedStudents = responseData.students_demographic_data.map(student => {
    ...
    if (progressRecord?.module_progress) {
      Object.entries(progressRecord.module_progress).forEach(([moduleName, progress]) => {
        // Try exact match first
        let moduleInfo = modulesByName[moduleName];
        
        // If not found, try without the module prefix
        if (!moduleInfo) {
          const cleanModuleName = moduleName.replace(/^(Pre-)?Module \d+:\s*/, '');
          moduleInfo = modulesByName[cleanModuleName];
        }
        
        // If still not found, search through all modules for partial match
        if (!moduleInfo) {
          const foundModule = moduleSettings.find(m => 
            m.name.includes(moduleName) || moduleName.includes(m.name.replace(/^(Pre-)?Module \d+:\s*/, ''))
          );
          if (foundModule) {
            moduleInfo = {
              ...foundModule,
...
export default function StudentProgress() {
  ...
  const getCompletionStats = useMemo(() => {
    ...
    const studentPercentages = Object.values(stats.byStudent).map(s => s.percentage);
    stats.overall = studentPercentages.length > 0 
      ? Math.round(studentPercentages.reduce((a, b) => a + b, 0) / studentPercentages.length)
      : 0;
  ...
  const exportToCSV = () => {
    ...
    // Prepare CSV data
    const csvRows = [];
    const headers = ['Student Name', 'Username', 'Overall Progress (%)', 'Modules Completed'];
    ...
    a.setAttribute('download', `student_progress_${courseData?.course_name || 'course'}_${new Date().toISOString().split('T')[0]}.csv`);
    a.click();
  };
  ...
  const ProgressBar = ({ percentage, size = 'default' }) => {
    ...
    const bgColor = percentage === 100 ? 'bg-green-500' : 
                     percentage > 50 ? 'bg-yellow-500' : 'bg-red-500';
    
    return (
      <div className={`flex items-center gap-2 ${size === 'small' ? 'w-20' : 'flex-1'}`}>
      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg mb-6">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            <div className="bg-gray-700/50 p-4 rounded-lg">
              ...
              <p className="text-2xl font-bold">{organizedData?.students.length || 0}</p>
            </div>
            <div className="bg-gray-700/50 p-4 rounded-lg">
            ...
            <div className="bg-gray-700/50 p-4 rounded-lg">
              ...
              <p className="text-2xl font-bold">{organizedData?.visibleModules.length || 0}</p>
            </div>
            <div className="bg-gray-700/50 p-4 rounded-lg">
              <p className="text-gray-400 text-sm mb-1">Active Quizzes</p>
              <p className="text-2xl font-bold">
                {organizedData ? Object.values(organizedData.quizzes).flat().length : 0}
              </p>
            </div>
          </div>
        </div>
        ...
        {activeTab === 'overview' && organizedData && (
          <div className="bg-gray-800 rounded-lg p-6">
            <h2 className="text-xl font-semibold mb-4">Module Completion Overview</h2>
                    ...
                    {organizedData.visibleModules.map(module => (
                      <th key={module.id} className="py-3 px-4 text-center">
                        <div className="text-xs">
                          {module.name.replace(/^(Pre-)?Module \d+:\s*/, '').substring(0, 15)}
                          {module.name.replace(/^(Pre-)?Module \d+:\s*/, '').length > 15 ? '...' : ''}
                        </div>
                      </th>
                    ))}
                  </tr>
                  ...
                  {organizedData.students.map(student => {
                    ...
                    return (
                      <tr key={student.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                        ...
                        <td className="py-3 px-4">
                          <ProgressBar percentage={studentStats?.percentage || 0} size="small" />
                        </td>
                        {organizedData.visibleModules.map(module => {
                          const moduleProgress = student.progress[module.module_rank];
                          if (!moduleProgress) {
                            ...
                            <td key={module.id} className="py-3 px-4">
                              <div className="flex justify-center">
                                <div className={`text-sm font-medium px-2 py-1 rounded ${
                                  percentage === 100 ? 'bg-green-600/30 text-green-400' :
                                  percentage > 0 ? 'bg-yellow-600/30 text-yellow-400' :
        ...
        {activeTab === 'quizzes' && organizedData && (
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="space-y-6">
              {/* Module Quizzes */}
              <div>
                <h3 className="text-lg font-medium mb-3 text-blue-300">Module Quizzes</h3>
                <div className="bg-gray-900/50 rounded-lg p-4">
                  {organizedData.quizzes.module.length > 0 ? (
                    <div className="space-y-3">
                      {organizedData.quizzes.module.map(quiz => {
                        const isQuizExpanded = expandedQuizzes[`quiz-${quiz.id}`];
                        // Get all grades for this quiz
                        const quizGrades = organizedData.allGrades.filter(g => g.quiz_id === quiz.id);
                          ...
                          <div key={quiz.id} className="bg-gray-800 rounded-lg overflow-hidden">
                            ...
                            >
                              <div className="flex items-center gap-3">
                                {isQuizExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                                <span className="font-medium">{quiz.name}</span>
                              </div>
                              <div className="flex items-center gap-4">
                                <span className="text-sm text-gray-400">
                                  {quizGrades.length} submission{quizGrades.length !== 1 ? 's' : ''}
                                </span>
                                <span className="text-sm text-gray-400">
                              ...
                              <div className="border-t border-gray-700 p-3">
                                {quizGrades.length > 0 ? (
                                  <div className="space-y-2">
                                    {organizedData.students.map(student => {
                                      const studentGrades = quizGrades.filter(g => g.student_id === student.id);
                                      if (studentGrades.length === 0) return null;
                                      
                                      const isStudentExpanded = expandedQuizzes[`quiz-${quiz.id}-student-${student.id}`];
                                      
                                      return (
                                        <div key={student.id} className="bg-gray-900/50 rounded p-2">
                                          <div 
                                          ...
                                          >
                                            <div className="flex items-center gap-2">
                                              {isStudentExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                                              <span className="text-sm">
                                                {student.student_first_name} {student.student_last_name}
                                              </span>
                                            </div>
                                            <span className="text-sm text-gray-400">
                                              {studentGrades.length} attempt{studentGrades.length !== 1 ? 's' : ''}
                                            </span>
                                          </div>
                                          ...
                                          {isStudentExpanded && (
                                            <div className="mt-2 ml-6 space-y-1">
                                              {studentGrades.map((grade, idx) => {
                                                const maxScore = grade.submitted_answers?.maxScore || quiz.total_score || 0;
                                                const percentage = maxScore > 0 ? Math.round((grade.score / maxScore) * 100) : 0;
                                                
                                                return (
                                                  <div key={grade.id} className="flex items-center gap-4 text-xs bg-gray-800 rounded p-2">
                                                    <span className="text-gray-400">Attempt #{idx + 1}</span>
                                                    <div className="flex items-center gap-2">
                                                      ...
                                                      <span className={`font-medium ${
                                                        ...
                                                        {grade.score}/{maxScore} ({percentage}%)
                                                      </span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
              ...
              {/* Surveys */}
              <div>
                <h3 className="text-lg font-medium mb-3 text-green-300">Surveys</h3>
                <div className="bg-gray-900/50 rounded-lg p-4">
                        ...
                        return (
                          <div key={quiz.id} className="bg-gray-800 rounded-lg overflow-hidden">
                            ...
                            >
                              ...
                              <span className="text-sm text-gray-400">
                                {quizGrades.length} response{quizGrades.length !== 1 ? 's' : ''}
                              </span>
                            </div>
                                      ...
                                      return (
                                        <div key={student.id} className="bg-gray-900/50 rounded p-2">
                                          <div className="flex items-center justify-between p-1">
                                            ...
                                            <span className="text-sm text-gray-400">
                                              Submitted: {new Date(studentGrades[0].time_submitted).toLocaleDateString()}
                                            </span>
                                          </div>
                                        </div>
                                      );
              ...
              {/* Pre/Post Tests */}
              <div>
                <h3 className="text-lg font-medium mb-3 text-purple-300">Pre/Post Tests</h3>
                <div className="bg-gray-900/50 rounded-lg p-4">
                                      ...
                                      return (
                                        <div key={student.id} className="bg-gray-900/50 rounded p-2">
                                          ...
                                          {isStudentExpanded && (
                                            <div className="mt-2 ml-6 space-y-1">
                                              {studentGrades.map((grade, idx) => {
                                                const maxScore = quiz.total_score || grade.submitted_answers?.maxScore || 0;
                                                const percentage = maxScore > 0 ? Math.round((grade.score / maxScore) * 100) : 0;
        ...
        {activeTab === 'detailed' && organizedData && (
          <div className="bg-gray-800 rounded-lg p-6">
            <h2 className="text-xl font-semibold mb-4">Detailed Student Progress</h2>
            <div className="space-y-4">
              {organizedData.students.map(student => {
                ...
                return (
                  <div key={student.id} className="bg-gray-900/50 rounded-lg overflow-hidden">
                    {/* Student Header */}
                    <div 
                      className="p-4 flex items-center justify-between cursor-pointer hover:bg-gray-700/30"
                      onClick={() => setExpandedRows(prev => ({...prev, [student.id]: !prev[student.id]}))}
                    >
                      <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        <ProgressBar percentage={studentStats?.percentage || 0} />
                      ...
                      <div className="p-4 pt-0">
                        <div className="bg-gray-800 rounded-lg p-4">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {organizedData.visibleModules.map(module => {
                                const progress = student.progress[module.module_rank];
                                ...
                                return (
                                  <tr key={module.id} className="border-b border-gray-700/30">
                                    ...
                                    <td className="py-2 text-center">
                                      <TaskIcon completed={progress.mini_lecture} />
                                    </td>
                                    <td className="py-2 text-center">
