...
export default function RotationTwoAxisQuiz() {
  ...
  useEffect(() => {
    try {
      ...
      if (sessionStorage.getItem("courseData") !== null) {
        ...
        // Check if quiz settings exist and find the "Rotation of Objects About Two or More Axes" quiz
        if (courseData?.settings?.quiz_settings) {
          const rotationOfObjectsQuiz = courseData.settings.quiz_settings.find(
            quiz => quiz.name === "Rotation of Objects About Two or More Axes"
          );

          if (rotationOfObjectsQuiz) {
