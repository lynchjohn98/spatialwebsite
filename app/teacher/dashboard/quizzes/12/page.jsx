...
import { quizData } from "../../../../library/quiz_data/orthographic_drawings_quiz";
import { submitStudentQuiz } from "../../../../library/services/student_services/student_quiz";

export default function OrthographicDrawingsQuiz() {
  ...
  useEffect(() => {
    try {
      ...
      if (sessionStorage.getItem("courseData") !== null) {
        ...
        // Check if quiz settings exist and find the "Orthographic Projection" quiz
        if (courseData?.settings?.quiz_settings) {
          const orthographicProjectionQuiz = courseData.settings.quiz_settings.find(
            quiz => quiz.name === "Orthographic Projection"
          );

          if (orthographicProjectionQuiz) {
            // Check visibility
            if (orthographicProjectionQuiz.visibility === "Yes") {
              setQuizVisible(true);
              setIsLoading(false);
            } else {
