...
import { quizData } from "../../../../library/quiz_data/datsr_post_quiz";
import { submitStudentPrePostQuiz } from "../../../../library/services/student_services/student_quiz";

export default function DATSRPostTest() {
  ...
  useEffect(() => {
    try {
      ...
      if (sessionStorage.getItem("courseData") !== null) {
        ...
        // Check if quiz settings exist and find the "DAT:SR Post-Test" quiz
        if (courseData?.settings?.quiz_settings) {
          const datsrPostTest = courseData.settings.quiz_settings.find(
            quiz => quiz.name === "DAT:SR Post-Test"
          );

          if (datsrPostTest) {
            // Check visibility
            if (datsrPostTest.visibility === "Yes") {
              setQuizVisible(true);
              setIsLoading(false);
            } else {
