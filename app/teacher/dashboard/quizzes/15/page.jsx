...
export default function DATSRPostQuiz() {
  ...
  useEffect(() => {
    try {
      ...
      if (quizSettings) {
        ...
        const datsrPostQuiz = quizSettings.find(
          quiz => quiz.name === "DAT:SR Post-Test"
        );

        console.log("DAT:SR Post-Test found:", datsrPostQuiz);

        if (datsrPostQuiz) {
          // Check visibility
          if (datsrPostQuiz.visibility === "Yes") {
            setQuizVisible(true);
            setIsLoading(false);
          } else {
